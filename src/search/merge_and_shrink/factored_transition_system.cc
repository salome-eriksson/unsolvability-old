...
void FactoredTransitionSystem::get_unsolvability_certificate(CuddBDD* h_inf,
                std::vector<CuddBDD> &bdd_for_val, bool fill_bdd_for_val) {
    heuristic_representations[final_index]->get_unsolvability_certificate(
                h_inf, bdd_for_val, fill_bdd_for_val);
}
}
